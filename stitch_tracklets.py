...
def main(FLAGS):
    ...
    for sequence in test_sequences:
        calib = parse_calibration(os.path.join(dataset, "sequences", seq_fmt.format(sequence), "calib.txt"))
        poses_f64 = parse_poses(os.path.join(dataset, "sequences", seq_fmt.format(sequence), "poses.txt"), calib)
        poses.append([pose.astype(np.float32) for pose in poses_f64])


    for poses_seq, sequence in zip(poses, test_sequences):
        point_names = []
        point_paths = os.path.join(dataset, "sequences", seq_fmt.format(sequence), "velodyne")
        ...
        if not os.path.exists(os.path.join(save_path, 'sequences', seq_fmt.format(sequence))):
            os.makedirs(os.path.join(save_path, 'sequences', seq_fmt.format(sequence)))

        if not os.path.exists(os.path.join(save_path, 'sequences', seq_fmt.format(sequence), 'predictions')):
            os.makedirs(os.path.join(save_path, 'sequences', seq_fmt.format(sequence), 'predictions'))
        ...
        for idx, point_file in zip(range(len(point_names)), point_names):
            ...
            #load current frame
            sem_path = os.path.join(prediction_path, (seq_fmt+'_{1:07d}.npy').format(sequence,idx))
            ins_path = os.path.join(prediction_path, (seq_fmt+'_{1:07d}_i.npy').format(sequence,idx))
            fet_path = os.path.join(prediction_path, (seq_fmt+'_{1:07d}_f.npy').format(sequence, idx))
            ...
            if FLAGS.nuscene:
                things = (label_sem_class < 11) & (label_sem_class > 0)
            else:
            ...
            if idx > 0:
                for i in range(1, n_test_frames):
                    ...
                    prev_inst_path = os.path.join(prediction_path, (seq_fmt+'_{1:07d}_{2}_i.npy').format(sequence, idx-i, idx))
                    prev_sem_path = os.path.join(prediction_path,
                                                  (seq_fmt+'_{1:07d}.npy').format(sequence, idx - i))

                    fet_path = os.path.join(prediction_path, (seq_fmt+'_{1:07d}_{2}_f.npy').format(sequence, idx - i, idx))
                    ...
                    prev_inst_orig_path = os.path.join(prediction_path,
                                                  (seq_fmt+'_{1:07d}_i.npy').format(sequence, idx - i))
                    prev_inst_orig = np.load(prev_inst_orig_path)

                    if FLAGS.nuscene:
                        things = (prev_sem < 11) & (prev_sem > 0)
                    else:
                    ...
                    prev_point_path = os.path.join(dataset, "sequences", seq_fmt.format(int(sequence)), "velodyne", '{0:06d}.bin'.format(idx-i))
                    #pose = poses[0][idx-i]
            ...
            new_preds.tofile(('{}/{}/'+seq_fmt_short+'/predictions/{:06d}.label').format(save_path, 'sequences', sequence, idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      ...
      type=str,
    #   required=True,
      default='SemanticKitti/dataset/semantic-kitti.yaml',
    )
    parser.add_argument(
      '--split',
      dest='split',
