...
from models.blocks import radius_gaussian, gather, max_pool, KPConv, BatchNormBlock
...
class KPConvInterSO3Block(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config) -> None:
        super().__init__()
...
class KPConvIntraSO3Block(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, layer_ind, config) -> None:
        super().__init__()
...
class SimpleBlockEPN(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config) -> None:
        ...
        self.interso3 = KPConvInterSO3Block(block_name, in_dim, self.out_dim, radius, layer_ind, config)
        if not self.non_sep_conv:
            self.intraso3 = KPConvIntraSO3Block(block_name, self.out_dim, self.out_dim, layer_ind, config)

    def forward(self, x, batch):
        x = self.interso3(x, batch)
        if not self.non_sep_conv:
            x = self.intraso3(x)
        return x
...
class ResnetBottleneckBlockEPN(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config) -> None:
        ...
        self.interso3 = KPConvInterSO3Block(block_name, out_dim // 4, out_dim // 4, radius, layer_ind, config)
        if not self.non_sep_conv:
            self.intraso3 = KPConvIntraSO3Block(block_name, out_dim // 4, out_dim // 4, layer_ind, config)
...
class InvOutBlockEPN(nn.Module):
    def __init__(self, block_name, in_dim, config, obj_mode=False) -> None: #, share_mode=False
        super().__init__()
        self.block_name = block_name
        self.in_dim = in_dim
        if obj_mode:
            self.rot_head_pool = config.rot_obj_pool
            self.att_pooling = 'attn' in self.rot_head_pool and not config.att_obj_permute #config.att_pooling #if hasattr(config, 'att_pooling') else True
            self.att_permute = 'attn' in self.rot_head_pool and config.att_obj_permute
        # elif share_mode:
        #     self.rot_head_pool = config.rot_head_pool
        #     self.att_pooling = 'attn' in self.rot_head_pool and not config.att_permute #config.att_pooling #if hasattr(config, 'att_pooling') else True
        #     self.att_permute = 'attn' in self.rot_head_pool and config.att_permute
        #     self.rot_obj_pool = config.rot_obj_pool
        #     self.att_obj_pooling = 'attn' in self.rot_obj_pool and not config.att_obj_permute #config.att_pooling #if hasattr(config, 'att_pooling') else True
        ...
        if self.att_permute:
            ...
            self.ridx_ori = nn.Parameter(ridx_ori, requires_grad=False)
    ...
    def forward(self, x, batch=None):
        ...
        if self.att_pooling or self.att_permute:
            ...
            attention = attention.squeeze(-1)   #p,a
            if self.att_pooling:
                if self.rot_head_pool == 'attn_soft':
                    x_fusion = (x * attention_w).sum(1) # p,c
                elif self.rot_head_pool == 'attn_best':
                    attention_w = torch.max(attention_w, 1, keepdim=True)[1].squeeze()  #np,na=1,nc=1
                    x_fusion = x[torch.arange(x.shape[0]), attention_w] # np, nc
                else:
                    raise NotImplementedError(f'self.rot_head_pool={self.rot_head_pool} not recognized') 
            else:
                x_permute = x[:, self.ridx_ori] #pac -> prac    # ridx_ori: used to recover original features from rotated ones
                x_permute = x_permute.flatten(2) # pr[ac]
                if self.rot_head_pool == 'attn_soft':
                    x_fusion = (x_permute * attention_w).sum(1)
                elif self.rot_head_pool == 'attn_best':
                    attention_w = torch.max(attention_w, 1, keepdim=True)[1].squeeze()  #np,na=1,nc=1
                    x_fusion = x_permute[torch.arange(x_permute.shape[0]), attention_w] # np, nc
                else:
                    raise NotImplementedError(f'self.rot_head_pool={self.rot_head_pool} not recognized') 
