...
class MPAnet(nn.Module):
    ...
    def __init__(
        ...
        self.main_proc_glob = rank == 0

        if config.saving:
            self.logger = logging.getLogger('main.'+__name__)

        if self.main_proc_glob:
            self.logger.info('NumberProposal: ' + str(num_proposal))
            self.logger.info('NumberSample: ' + str(nsample))
            self.logger.info('NumberSampleSub: ' + str(nsample_sub))
            self.logger.info('Radius: ' + str(radius))
        ...
        point_feature_dim = self.backbone_net.feature_dim

        # Hough voting
        self.kanchor = config.kanchor
        self.vgen = VotingModule(vote_factor=1, seed_feature_dim=config.first_features_dim, 
                                config=config, kanchor=self.kanchor)
        obj_feature_dim = self.vgen.out_dim

        # Vote aggregation and proposal generation
        if self.nuscene:
            self.pnet = ProposalModule(10, num_proposal, obj_feature_dim, radius, nsample, nsample_sub, use_fps, use_geo_features, use_binary_mask_in_proposal_module)
        else:
            self.pnet = ProposalModule(8, num_proposal, obj_feature_dim, radius, nsample, nsample_sub, use_fps, use_geo_features, use_binary_mask_in_proposal_module)

        # Semantic classes for background points (FP background points)
        if self.backbone_net.effective_kanchor == 1:
            if self.nuscene:
                self.sem_background_net = UnaryBlock(point_feature_dim, 6, False, 0)
            else:
                self.sem_background_net = UnaryBlock(point_feature_dim, 11, False, 0)
        else:
            if self.nuscene:
                self.sem_background_net = UnaryBlockEPN(point_feature_dim, 6, False, 0)
            else:
                self.sem_background_net = UnaryBlockEPN(point_feature_dim, 11, False, 0)

        if use_mask_module:
            self.mask_net = MaskModule(mlp=[point_feature_dim + 128, 256, 128, 64, 32, 2])
        ...
        self.rot_cls_loss = torch.tensor(0)
        self.rot_cls_acc = torch.tensor(0)
    ...
    def forward(self, batch):
        ...
        point_semantic_classes, point_objectness_scores, point_features, point_features_obj, rot_attn = self.backbone_net(batch)
        ### nc, nc, n(a)c, na
        if point_features.ndim ==3:
            point_features = point_features.transpose(1,2)  # nac->nca
        ...
        selected_point_semantic_classes_ = selected_point_semantic_classes[None, :]    # B,N

        # Reshape to have a minibatch size of 1
        point_features = torch.transpose(point_features, 0, 1)  # N,C(,A) - C,N(,A)
        point_features = point_features.unsqueeze(0)
        point_positions = point_positions.unsqueeze(0)

        if point_features_obj is None:
            point_features_obj = point_features
        else:
            if point_features_obj.ndim ==3:
                point_features_obj = point_features_obj.transpose(1,2)  # nac->nca
            point_features_obj = torch.transpose(point_features_obj, 0, 1)  # N,C(,A) - C,N(,A)
            point_features_obj = point_features_obj.unsqueeze(0)

        #t_start_vgen_forward_pass = time.time()
        normalization = True
        # Need to be consistent during training and inference
        if normalization:
            point_votes, point_votes_features, rot_cls = self.vgen(point_positions, point_features_obj, selected_point_semantic_classes_, rot_attn)
            ...
            point_votes, _, _ = self.vgen(point_positions, point_features_obj)
            point_votes_features = point_features_obj.contiguous()
        ...
        #     object_point_rot_cls = rot_cls[:, :, object_points_ids]
        ...
        if self.backbone_net.effective_kanchor == 1:
            ...
            fp_points_features = fp_points_features.permute(1,2,0)    # CNA->NAC
            fp_points_sem_classes = self.sem_background_net(fp_points_features)
            
            if self.backbone_net.rot_head_pool in ['attn_soft', 'attn_best']:
                if self.backbone_net.rot_head_pool == 'attn_soft':
                    rot_attn = F.softmax(rot_attn, 1).unsqueeze(-1) # na->na1
                    fp_points_sem_classes = (fp_points_sem_classes * rot_attn).sum(1)
                else:
                    rot_attn = torch.max(rot_attn, 1)[1].squeeze()  #n
                    fp_points_sem_classes = fp_points_sem_classes[torch.arange(fp_points_sem_classes.shape[0]), rot_attn] # np, nc
            elif self.backbone_net.rot_head_pool == 'mean':
                fp_points_sem_classes = fp_points_sem_classes.mean(1)
            elif self.backbone_net.rot_head_pool == 'max':
                ### max does not make much sense here? since they are all predictions
                fp_points_sem_classes = fp_points_sem_classes.amax(1)
            else:
                raise NotImplementedError(f'self.backbone_net.rot_head_pool={self.backbone_net.rot_head_pool} not recognized') 
            # fp_points_sem_classes = fp_points_sem_classes.mean(1)   # NAC -> NC
        
        if self.distributed and self.training:
            fp_points_sem_classes = None
            if self.pre_train:
                proposal_semantic_classes = None
                proposal_positions = None
                proposal_objectness_scores = None
                proposal_aggregation_features = None
                proposal_binary_mask = None
                proposal_ids = None
            else:
                point_semantic_classes = point_semantic_classes.detach()
                point_votes = None
                point_objectness_scores = None
                rot_cls = None
             
        return proposal_binary_mask, fp_points_sem_classes, proposal_semantic_classes, proposal_aggregation_features, proposal_objectness_scores, proposal_positions, proposal_ids, point_semantic_classes, point_votes, point_objectness_scores, rot_cls


    def loss(self, proposal_binary_mask, point_semantic_classes, point_votes, point_objectness_scores, rot_cls, proposal_ids, proposal_objectness_scores, proposal_positions, proposal_semantic_classes, proposal_aggregation_features, fp_points_sem_classes, labels, ins_labels, centers_gt, points=None, times=None):
        """
        ...
        if self.pre_train:
            ...
            if object_points_ids.shape[0] > 0:
                ...
                ### rot_cls cross entropy loss
                # Reshape rot_cls_gt to have a minibatch size of 1
                if rot_cls is not None:
                    rot_cls_gt = centers_gt[object_points_ids, 11].unsqueeze(0)  # 1, N
                    rot_cls = rot_cls[:, :, object_points_ids]
                    rot_cls_loss_fn = nn.CrossEntropyLoss()
                    self.rot_cls_loss = rot_cls_loss_fn(rot_cls, rot_cls_gt.long())
                    _, rot_cls_max = torch.max(rot_cls, 1)
                    self.rot_cls_acc = (rot_cls_max == rot_cls_gt).float().mean()
            else:
                self.point_center_regression_loss = point_votes.mean()*0
                if rot_cls is not None:
                    self.rot_cls_loss = rot_cls.mean()*0
        ...
        if not self.pre_train:
            if object_points_ids.shape[0] > 0:
                ...
                if help_ids.shape[0] > 0:
                    ...
                    criterion_sem_cls = nn.CrossEntropyLoss(ignore_index=-1)#!!!
        ...
        # COMBINED LOSS
        loss = self.point_semantic_classification_loss + self.point_objectness_loss + self.point_center_regression_loss + self.proposal_objectness_loss + self.proposal_semantic_loss + self.fp_points_sem_loss + self.proposal_agg_feature_loss + self.proposal_mask_loss + self.rot_cls_loss
        return loss
    ...
    def ins_pred_in_time_mpa(self, labels, proposals_binary_mask, fp_points_sem_classes, proposals_aggregation_features, proposals_semantic_classes, proposals_objectness_scores, proposals_positions, proposals_ids, point_semantic_classes, point_positions, next_ins_id):
        ...
        if use_objectness_scores:
            self.logger.debug('use_objectness_scores')
        ...
        if use_binary_mask:
            self.logger.debug('use_binary_mask')
        ...
        if use_majority_voting:
            self.logger.debug('use_majority_voting')
