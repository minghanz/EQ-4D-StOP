...
class ModelTrainer:
    ...
    def __init__(self, net, config, chkp_path=None, finetune=False, on_gpu=True, gpu_id=0, rank=0, distributed=False):
        """
        ...
        if (chkp_path is not None):
            if finetune:
                ...
                if self.main_proc_glob:
                    self.logger.info("Model restored and ready for finetuning.")
                    self.logger.info(f"chkp_path: {chkp_path}")
            elif config.freeze:
                checkpoint = torch.load(chkp_path, map_location=self.device)
                ...
                #net.train()
                if self.main_proc_glob:
                    self.logger.info("Model and training state restored.")
                    self.logger.info(f"chkp_path: {chkp_path}")
            else:
        ...
        # Path of the result folder
        if config.saving and self.main_proc_glob:
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                ...
                if (t[-1] - last_display) > 1.0:
                    ...
                    message = 'e{:03d}-i{:04d} => L={:.3f} L_O={:.3f} L_C={:.3f} L_I={:.3f} L_V={:.3f} acc={:3.0f}% {:2.1f}GB'
    ...
    def train_mpa(self, net, training_loader, val_loader, config):
        ...
        if config.saving:
            if self.main_proc_glob:
                timestamp = config.saving_path.split('/')[-1]
                writer = SummaryWriter('runs/'+timestamp)
                # Training log file
                with open(join(config.saving_path, 'training.txt'), "w") as file:
                    file.write('epochs steps loss out_loss center_loss instance_loss variance_loss train_accuracy rot_accuracy mem time\n')
        ...
        # for epoch in range(config.max_epoch):
        while True:
            ### change the meaning of config.max_epoch to the global number
            ...
            rot_cls_loss_summed = 0
            rot_cls_acc_summed = 0

            if self.distributed:
                training_loader.sampler.set_epoch(self.epoch)
            ...
            for batch in training_loader:
                ...
                # Forward pass
                proposal_binary_mask, fp_points_sem_classes, proposal_semantic_classes, proposal_aggregation_features, proposal_objectness_scores, proposal_positions, proposal_ids, outputs, point_votes, point_objectness_scores, rot_cls = net(batch)
                net_single = net.module if self.distributed else net
                loss = net_single.loss(proposal_binary_mask, outputs, point_votes, point_objectness_scores, rot_cls, proposal_ids, proposal_objectness_scores, proposal_positions, proposal_semantic_classes, proposal_aggregation_features, fp_points_sem_classes, batch.labels, batch.ins_labels, batch.centers, batch.points[0], batch.times.unsqueeze(1))
                
                acc = net_single.accuracy(outputs, batch.labels)

                acc_summed = acc_summed + acc
                loss_summed = loss_summed + loss.item()
                point_semantic_classification_loss_summed = point_semantic_classification_loss_summed + net_single.point_semantic_classification_loss.item()
                point_objectness_loss_summed = point_objectness_loss_summed + net_single.point_objectness_loss.item()
                point_center_regression_loss_summed = point_center_regression_loss_summed + net_single.point_center_regression_loss.item()
                proposal_objectness_loss_summed = proposal_objectness_loss_summed + net_single.proposal_objectness_loss.item()
                proposal_semantic_loss_summed = proposal_semantic_loss_summed + net_single.proposal_semantic_loss.item()
                fp_points_sem_loss_summed = fp_points_sem_loss_summed + net_single.fp_points_sem_loss.item()
                proposal_agg_feature_loss_summed = proposal_agg_feature_loss_summed + net_single.proposal_agg_feature_loss.item()
                proposal_mask_loss_summed = proposal_mask_loss_summed + net_single.proposal_mask_loss.item()
                instance_loss_summed = instance_loss_summed + net_single.instance_loss.item()
                variance_loss_summed = variance_loss_summed + net_single.variance_loss.item()

                rot_cls_loss_summed = rot_cls_loss_summed + net_single.rot_cls_loss.item()
                rot_cls_acc_summed = rot_cls_acc_summed + net_single.rot_cls_acc.item()
                ...
                if self.main_proc_glob:
                    # Console display (only one per 10 second)
                    # if True:
                    if self.epoch < 5 or (t[-1] - last_display) > 10.0:
                        last_display = t[-1]
                        mem = torch.cuda.max_memory_allocated()/1024/1024/1024
                        torch.cuda.reset_peak_memory_stats()
                        message = 'e{:03d}-i{:04d} => L={:.3f} L_O={:.3f} L_C={:.3f} L_I={:.3f} L_V={:.3f} acc={:3.0f}% acc_R={:3.0f}% {:2.1f}GB / t(ms): {:5.1f} {:5.1f} {:5.1f}'
                        self.logger.info(message.format(self.epoch + 1, self.step + 1,
                                            loss.item(),
                                            net_single.point_semantic_classification_loss.item(),
                                            net_single.point_objectness_loss.item(),
                                            net_single.instance_loss.item(),
                                            net_single.variance_loss.item(),
                                            100 * acc,
                                            100*net_single.rot_cls_acc.item(),
                                            mem,
                                            1000 * mean_dt[0],
                                            1000 * mean_dt[1],
                                            1000 * mean_dt[2],
                        ...
                        if config.saving:
                            with open(join(config.saving_path, 'training.txt'), "a") as file:
                                message = '{:d} {:d} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.3f} {:.1f} {:.3f}\n'
                                file.write(message.format(self.epoch + 1,
                                                        self.step + 1,
                                                        loss,
                                                        net_single.point_semantic_classification_loss,
                                                        net_single.point_objectness_loss,
                                                        net_single.instance_loss,
                                                        net_single.variance_loss,
                                                        acc,
                                                        net_single.rot_cls_acc,
                                                        mem,
                                                        t[-1] - t0))
            ...
            if config.saving and self.main_proc_glob:
                ...
                writer.add_scalar("Variance_Loss", variance_loss_summed / config.epoch_steps, self.epoch + 1)
                writer.add_scalar("Accuracy", acc_summed / config.epoch_steps, self.epoch + 1)
                writer.add_scalar("Rotation_Classification_Loss", rot_cls_loss_summed / config.epoch_steps, self.epoch + 1)
                writer.add_scalar("Rotation_Accuracy", rot_cls_acc_summed / config.epoch_steps, self.epoch + 1)
                i = 0
                for param_group in self.optimizer.param_groups:
                    writer.add_scalar("Learning_Rate_" + str(i), param_group['lr'], self.epoch + 1)
                    i += 1
            ...
            if config.saving and self.main_proc_glob:
                # Get current state dict
                net_state_dict = net.module.state_dict() if self.distributed else net.state_dict()
                save_dict = {'epoch': self.epoch + 1,
