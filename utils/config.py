...
class Config:
    ...
    # whether or not use a equivariant network
    equivariant_mode = False
    # whether or not use the kernel file from epn
    epn_kernel = False  # always false
    # number of rotation anchors A
    kanchor = 1
    # early fusion with attentive averge pooling
    att_pooling = False
    # early fusion with soft permutation
    att_permute = False
    # use both permutation and max pooling as the invariant layer
    dual_feature = False
    # use the max pooling feature to regress centerness
    ctrness_w_track = False
    # noise free kernel
    # combined with fixed_kernel_points = 'verticals' results in z-axis symmetric kernel
    equiv_mode_kp = False
    # full (non-separable) conv
    non_sep_conv = False
    # rotate-by-permute in full conv
    rot_by_permute = False
    # > 1 enables quotient features. 2 results in C6/C2. 
    quotient_factor = 1
    # late fusion with attention weight. Need to remove 'inv_epn' to enable late fusion. 
    # rot_head_attn = False
    rot_head_pool = 'mean' # attn_best, attn_soft, mean, max
    rot_obj_pool = 'attn_best'
    att_obj_permute = False
    early_fuse_obj = False
    share_fuse_obj = False
    # ignore steerability constraint when using quotient features
    ignore_steer_constraint = False
    # efficient gathering
    gather_by_idxing = False
    # separate rotation classification for different semantic classes
    rot_semantic_cls = 1
    # separate offset regression for different semantic classes
    ...
    def save(self):
        ...
        with open(join(self.saving_path, 'parameters.txt'), "w") as text_file:
            ...
            # Training parameters
            text_file.write('# Equivariance parameters\n')
            text_file.write('# *******************\n\n')

            # text_file.write('reinit_var = {:d}\n'.format(self.reinit_var))
            text_file.write('equivariant_mode = {:d}\n'.format(self.equivariant_mode))
            text_file.write('epn_kernel = {:d}\n'.format(self.epn_kernel))
            text_file.write('kanchor = {:d}\n'.format(self.kanchor))
            text_file.write('att_pooling = {:d}\n'.format(self.att_pooling))
            text_file.write('att_permute = {:d}\n'.format(self.att_permute))
            text_file.write('dual_feature = {:d}\n'.format(self.dual_feature))
            text_file.write('ctrness_w_track = {:d}\n'.format(self.ctrness_w_track))
            text_file.write('equiv_mode_kp = {:d}\n'.format(self.equiv_mode_kp))
            text_file.write('non_sep_conv = {:d}\n'.format(self.non_sep_conv))
            text_file.write('rot_by_permute = {:d}\n'.format(self.rot_by_permute))
            text_file.write('quotient_factor = {:d}\n'.format(self.quotient_factor))
            text_file.write('rot_head_pool = {:s}\n'.format(self.rot_head_pool))           
            text_file.write('rot_obj_pool = {:s}\n'.format(self.rot_obj_pool))     
            text_file.write('att_obj_permute = {:d}\n'.format(self.att_obj_permute))  
            text_file.write('early_fuse_obj = {:d}\n'.format(self.early_fuse_obj))  
            text_file.write('share_fuse_obj = {:d}\n'.format(self.share_fuse_obj))  
            text_file.write('ignore_steer_constraint = {:d}\n'.format(self.ignore_steer_constraint))
            text_file.write('gather_by_idxing = {:d}\n'.format(self.gather_by_idxing))   
            text_file.write('rot_semantic_cls = {:d}\n'.format(self.rot_semantic_cls))
            text_file.write('offset_semantic_cls = {:d}\n'.format(self.offset_semantic_cls))
            text_file.write('train_val = {:d}\n'.format(self.train_val))
            text_file.write('neq_rot_cls = {:d}\n'.format(self.neq_rot_cls))
            text_file.write('neq_kanchor = {:d}\n'.format(self.neq_kanchor))
            text_file.write('nuscene = {:d}\n'.format(self.nuscene))
