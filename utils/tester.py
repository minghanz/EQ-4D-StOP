...
class ModelTester:
    ...
    def __init__(self, net, chkp_path=None, on_gpu=True, saving=True):
    ...
    def panoptic_4d_test_mpa(self, net, test_loader, config, num_votes=100, debug=True, vis=False):
        """
        Test method for slam segmentation models
        """
        ...
        if config.saving:
            test_path = config.saving_path
            open(join(config.saving_path, config.assoc_saving+str(config.n_test_frames) + '_' + config.chosen_chkp), 'x').close()
            # test_path = join(config.test_path, config.saving_path.split('/')[-1]+ '_'+config.assoc_saving+str(config.n_test_frames) + '_' + config.chosen_chkp)
            if not exists(test_path):
                makedirs(test_path)
            report_path = join(test_path, 'reports')
        ...
        while True:
            ...
            for i, batch in enumerate(test_loader):
                ...
                if i == 0:
                    self.logger.info('Done in {:.1f}s'.format(t[1] - t[0]))
                ...
                with torch.no_grad():
                    ...
                    if binary:
                        ...
                        t_end_forward_pass = time.time()
                        duration_forward_pass = t_end_forward_pass - t_start_forward_pass
                        self.logger.debug('duration_forward_pass: ' + str(duration_forward_pass))
                    else:
                        t_start_forward_pass = time.time()
                        proposal_binary_mask, fp_points_sem_classes, proposal_semantic_classes, proposal_aggregation_features, proposal_objectness_scores, proposal_positions, proposal_ids, outputs, point_votes, centers_output, rot_cls = net(batch)
                        t_end_forward_pass = time.time()
                        duration_forward_pass = t_end_forward_pass - t_start_forward_pass
                        self.logger.debug('duration_forward_pass: ' + str(duration_forward_pass))
                    ...
                    t_end_ins_pred = time.time()
                    duration_ins_pred = t_end_ins_pred - t_start_ins_pred
                    self.logger.debug('duration_ins_pred: ' + str(duration_ins_pred))

                    duration_relevant = duration_forward_pass + duration_ins_pred
                    self.logger.debug('duration_relevant: ' + str(duration_relevant))

                    duration_relevant_summed = duration_relevant_summed + duration_relevant
                    self.logger.debug('duration_relevant_summed: ' + str(duration_relevant_summed))
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    point_votes = point_votes.squeeze()
                    point_votes = point_votes.cpu().detach().numpy()
                    ### Preserve only instance-related votes. If we want to visualize the offset vectors, comment below two lines
                    # ins_ids = np.where((point_semantic_classses < 9) & (point_semantic_classses != 0))[0]
                    # point_votes = point_votes[ins_ids]
                    if vis:
                        np.save(filepath_v_4d, point_votes)
                ...
                duration_step = t_end - t_start_step
                self.logger.debug('duration_step: ' + str(duration_step))

                duration_inference = t_end - t_start_inference
                self.logger.debug('duration_inference: ' + str(duration_inference))
                ...
                if (t[-1] - last_display) > 10.0:
                    ...
                    message = 'e{:03d}-i{:04d} => {:.1f}% (timings : {:4.2f} {:4.2f} {:4.2f}) / pots {:d} => {:.1f}% {:2.1f}GB'
                    min_pot = int(torch.floor(torch.min(test_loader.dataset.potentials)))
                    pot_num = torch.sum(test_loader.dataset.potentials > min_pot + 0.5).type(torch.int32).item()
                    current_num = pot_num + (i + 1 - config.validation_size) * config.val_batch_num
                    self.logger.info(message.format(test_epoch, i,
                                         100 * i / config.validation_size,
                                         1000 * (mean_dt[0]),
                                         1000 * (mean_dt[1]),
                                         1000 * (mean_dt[2]),
            ...
            # Update minimum od potentials
            new_min = torch.min(test_loader.dataset.potentials)
            self.logger.info('Test epoch {:d}, end. Min potential = {:.1f}'.format(test_epoch, new_min))
            ...
            if last_min + 1 < new_min:
                ...
                if test_loader.dataset.set == 'validation' and last_min % 1 == 0:
                    ...
                    C_tot = fast_confusion(val_labels, val_preds, test_loader.dataset.label_values)
                    t3 = time.time()
                    self.logger.info(' Stacking time : {:.1f}s'.format(t2 - t1))
                    self.logger.info('Confusion time : {:.1f}s'.format(t3 - t2))
