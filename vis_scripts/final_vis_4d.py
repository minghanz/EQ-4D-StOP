...
def main():
    ...
    frame_number = '0000028'
    seq_number = '08'
    # log_folder = 'Log_2023-02-25_10-25-22/vis_vote_1100'    # eq-4d-stop, 128x4
    log_folder = 'Log_2023-02-08_23-31-15/vis_vote_1100'    # 4d-stop, 256x1

    path = 'test/' + log_folder + '/4D_val_probs/' + seq_number + '_' + frame_number

    panoptic_visualization = True
    semantic_visualization = True
    ...
    if use_lines:
        ...
        for id in instance_ids:
            ...
            if use_votes:
                # Draw offset vectors
                point_votes_ = point_votes[idx]
                v.add_lines('lines' + str(id), instance_points[chosen_ids], point_votes_[chosen_ids])
            else:
                # Draw connections between points and instance center
                v.add_lines('lines' + str(id), instance_points[chosen_ids], bb_centers[chosen_ids])
    

    path_exp = 'test/' + log_folder
    if path_exp[-1] != '/':
        path_exp = path_exp + '/'
