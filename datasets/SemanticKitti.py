...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True, seqential_batch = False, train_val=False):
        ...
        # Check if it is nuscenes dataset in semantickitti format
        self.nuscene = config.nuscene
        ...
        if self.nuscene:
            ...
            if self.set == 'training':
                if train_val:
                    self.sequences = ['{:04d}'.format(i) for i in splits['train']+splits['valid']]
                else:
                    self.sequences = ['{:04d}'.format(i) for i in splits['train']]
                print(self.sequences)
            elif self.set == 'validation':
                self.sequences = ['{:04d}'.format(i) for i in splits['valid']]
                print(self.sequences)
            elif self.set == 'test':
                self.sequences = ['{:04d}'.format(i) for i in splits['test']]
                print(self.sequences)
            else:
        ...
        self.quotient_factor = config.quotient_factor
        if config.neq_rot_cls:
            self.kanchor = config.neq_kanchor
        self.init_anchors()
        ...
        ### use deterministic sampling so that the num_workers of dataset instances have the same initialization
        ### which is necessary for multi-GPU training to have non-overlaping batches
        ### because share_memory_() is not effective across different GPUs in DDP. 
        rng = np.random.default_rng(0)
        # self.potentials = torch.from_numpy(np.random.rand(self.all_inds.shape[0]) * 0.1 + 0.1)
        self.potentials = torch.from_numpy(rng.random(self.all_inds.shape[0]) * 0.1 + 0.1)
        if seqential_batch:
    ...
    def init_anchors(self):
        ...
        if self.quotient_factor == 1:
            if self.kanchor < 10:
                ...
                self.anchors_s2 = self.anchors[:,:,0] # k*3
            else:
                assert self.kanchor == 60, self.kanchor
                self.anchors_s2, _, _, _, self.anchors = L.get_icosahedron_vertices()
        else:
            if self.kanchor < 10:
                ...
                self.quotient_anchors = np.array(quotient_anchors, dtype=np.float32)
            else:
                assert self.kanchor == 12, self.kanchor
                assert self.quotient_factor == 5, self.quotient_factor
                self.anchors_s2, _, _, _, anchors = L.get_icosahedron_vertices()
                self.anchors = anchors.reshape(12, 5, 3, 3)[:, 0]
                quotient_anchors = L.get_anchors(self.quotient_factor)
                self.quotient_anchors = np.array(quotient_anchors, dtype=np.float32)
        return
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set == 'test':
                    ...
                    center_labels = np.load(center_file)[:,[0]]
                ...
                if self.set in ['training', 'validation']  and f_inc > 0 and  self.config.n_test_frames == 1:#during training
                    ...
                    if self.nuscene:
                        obj_idx_min_exclude = 0
                        obj_idx_max_exclude = 11
                    else:
                        obj_idx_min_exclude = 0
                        obj_idx_max_exclude = 9

                    if self.config.sampling == 'objectness':
                        mask = ((sem_labels > obj_idx_min_exclude) & (sem_labels < obj_idx_max_exclude) & mask)
                    elif self.config.sampling == 'importance':
                        n_points_to_sample = np.sum((sem_labels > obj_idx_min_exclude) & (sem_labels < obj_idx_max_exclude))
                ...
                if self.set in ['validation', 'test'] and self.config.n_test_frames > 1 and f_inc > 0:
                    ...
                    test_path = self.config.saving_path
                    ...
                    if self.config.sampling == 'objectness':
                        ...
                        while center_pred is None:
                            try:
                                ...
                                counter +=1
                                if counter > 5:
                                    print("center_pred file_path not found: {}".format(file_path))
                                    break
                                continue
            ...
            in_cts = np.zeros((in_fts.shape[0], 12))
            ...
            if self.set == 'training':
                ...
                center_gt, radius_gt, bb_size_gt, offset_dir_gt = get_center_and_radius_gt(in_pts, in_slbls, in_lbls, self.anchors_s2)
                in_cts[:, 1:7] = center_gt[:, 1:7]
                in_cts[:, 7] = radius_gt[:, 0]
                in_cts[:, 8:11] = bb_size_gt[:, 0:3]
                in_cts[:, 11] = offset_dir_gt[:, 0]
...
class SemanticKittiSamplerDistributed(SemanticKittiSampler):
    def __init__(self, dataset: SemanticKittiDataset, num_replicas: Optional[int] = None,
        ...
        self.num_samples = self.dataset.epoch_inds.shape[0]  # type: ignore[arg-type]
    ...
    def __iter__(self):
        ...
        if self.dataset.balance_classes:
            ...
            # Initiate current epoch ind
            ### index of sample in an epoch
            self.dataset.epoch_i *= 0
            ### Sampled indices of frames in this epoch
            self.dataset.epoch_inds *= 0
            self.dataset.epoch_labels *= 0

            # Number of sphere centers taken per class in each cloud
            ### The number of frames needed in an epoch
            num_centers = self.dataset.epoch_inds.shape[0] * self.num_replicas
            ### Need to consider all GPUs here. 
            num_centers_single_gpu = self.dataset.epoch_inds.shape[0]
            ...
            for i, c in enumerate(self.dataset.label_values):
                if c not in self.dataset.ignored_labels:
                    ...
                    # Get the indices to generate thanks to potentials
                    used_classes = self.dataset.num_classes - len(self.dataset.ignored_labels)
                    ### The number of frames needed that contains a certain class
                    class_n = num_centers // used_classes + 1
                    if class_n < class_potentials.shape[0]:
                        ### Sample the requested number of frames from all qualified frames
                        _, class_indices = torch.topk(class_potentials, class_n, largest=False)
                    else:
                        ### Take all qualified frames
                        class_indices = torch.randperm(class_potentials.shape[0], generator=g)
                    class_indices = self.dataset.class_frames[i][class_indices]
                    ...
                    # Update potentials
                    self.dataset.potentials[class_indices] = np.ceil(self.dataset.potentials[class_indices])
                    self.dataset.potentials[class_indices] += rng.random(class_indices.shape[0]) * 0.1 + 0.1
            ...
            # Shuffle generated indices
            rand_order = torch.randperm(gen_indices.shape[0], generator=g)[:num_centers]
            if num_centers > gen_indices.shape[0]:
                extra_order = torch.randperm(num_centers-gen_indices.shape[0], generator=g)
                rand_order = torch.cat((rand_order, extra_order), dim=0)
            gen_indices = gen_indices[rand_order]
            gen_classes = gen_classes[rand_order]
            ...
            # Update potentials (Change the order for the next epoch)
            self.dataset.potentials[gen_indices] = torch.ceil(self.dataset.potentials[gen_indices])
            self.dataset.potentials[gen_indices] += torch.from_numpy(rng.random(gen_indices.shape[0]) * 0.1 + 0.1)

            # Update epoch inds
            gen_indices_single_gpu = gen_indices[self.rank : num_centers : self.num_replicas]
            gen_classes_single_gpu = gen_classes[self.rank : num_centers : self.num_replicas]
            assert len(gen_indices_single_gpu) == num_centers_single_gpu, \
                "{}, {}".format(len(gen_indices_single_gpu), num_centers_single_gpu)
            assert len(gen_classes_single_gpu) == num_centers_single_gpu, \
                "{}, {}".format(len(gen_classes_single_gpu), num_centers_single_gpu)
            
            self.dataset.epoch_inds += gen_indices_single_gpu
            self.dataset.epoch_labels += gen_classes_single_gpu.type(torch.int32)
            ...
            assert not self.dataset.seqential_batch, 'seqential_batch is not supported in multi-GPU. '
            ...
            if num_centers < self.dataset.potentials.shape[0]:
                ...
                gen_indices = torch.randperm(self.dataset.potentials.shape[0], generator=g)

            self.dataset.potentials[gen_indices] = torch.ceil(self.dataset.potentials[gen_indices])
            self.dataset.potentials[gen_indices] += torch.from_numpy(rng.random(gen_indices.shape[0]) * 0.1 + 0.21)
...
#added by me
def get_center_and_radius_gt(points, ins_labels, labels, anchors_s2=None):
    ...
    offset_dir_labels = np.zeros((points.shape[0], 1))
    for _, semins in enumerate(sem_ins_labels):
        ...
        for _, sem_label in enumerate(sem_labels):
            ...
            # Compute direction
            if anchors_s2 is not None:
                offsets = center_labels[valid_ind2, 1:4][None]  # 1, n, 3
                anchors = anchors_s2[:,None]    # k, 1, 3
                inner_p = (offsets * anchors).sum(-1)   # k,n
                best_anchor = np.argmax(inner_p, 0) # n
                offset_dir_labels[valid_ind2, 0] = best_anchor


    return center_labels, radius_labels, bb_size_labels, offset_dir_labels
#end added
